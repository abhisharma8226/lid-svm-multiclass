...
def draw_contours(X, y, model, fi, title, file_name):
    pos_indices = np.where(y == 1)[0] # returns position 
...
def classification_perf(y, y_hat, scenario, index, column_names):
    ...
    return pd.DataFrame([[scenario, index, accuracy, error_rate, precision, np.average(precision),
                          recall, np.average(recall), fscore, np.average(fscore)]],
...
def mle_batch_kernel(data, batch, gamma, k):
    ...
    k = min(k, len(data) - 1)
    f = lambda v: - k / np.sum(np.log(v / v[-1]))
    # f = lambda v: -k / (np.sum(np.log(v / (v[-1] + 1e-10))) + 1e-10)
...
def weight_calculation(normal_lids, fl_lids, density_normal, density_fl, WEIGHT_LB):
    # density of normal lids from the normal distributionF
